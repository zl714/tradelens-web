...
Shared design system for TradeLens (blue accent) and Diamond Mind (cyan accent). Navy near-black, Geist, slate grays, tabular numbers. Clean professional dashboards — NOT green, NOT vibe-coded.
...
:root {
  ...
  /* ---- ACCENT (interactive: links / primary btn / focus / active).
          DEFAULT below = TradeLens electric blue. DM overrides it. ---- */
  --accent-300: #93C5FD;
...
- STREAKS / consistency: a current-streak counter (small badge + bold tabular count) and a GitHub-style consistency heatmap of logged sessions using a 5-level ramp from --surface-3 up to --accent-500 (cyan), ~11px cells — ties the coaching cadence to the same progress-viz language TradeLens uses for trends.
